...
I've just found a script on another blog to go through all the clustered indexes in a SQL Server database and rebuild them in order to reduce fragmentation and improve the disk IO needed to get the data. The <a href="http://www.keepitsimpleandfast.com/2008/12/optimize-index-structure.html">original script</a> didn't take into account tables in different schemas so I updated it. I also added some metrics to it so I could get a sense of how long the operation takes on each table.
